...
function GlobalMap() { 
  ...
  useEffect(() => {
    try { findUserLocations(); } catch (error) { console.log("location error", error); }
  }, []); 
          ...
          <Popup latitude={Number(selectedLoc.latitude)} longitude={Number(selectedLoc.longitude)} anchor="top" onClose={() => setSelectedLoc(null)}>
            <div style={{ direction: 'rtl', textAlign: 'right', fontFamily: 'sans-serif', padding: '5px', color: '#333' }}>
              ...
              <div style={{ marginTop: '12px', borderTop: '1px solid #eee', paddingTop: '8px', display: 'flex', gap: '10px' }}>
                <a href={`https://google.com{selectedLoc.latitude},${selectedLoc.longitude}`} target="_blank" rel="noopener noreferrer" style={{ color: '#1a73e8', fontWeight: 'bold', textDecoration: 'none', fontSize: '13px' }}>ניווט ➔</a>
                <button onClick={() => handlSave(selectedLoc)} style={{ padding: '4px 8px', backgroundColor: '#1a73e8', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>שמור מקום</button>
              </div>
            </div>
          </Popup>
        )}
