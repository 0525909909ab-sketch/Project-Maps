...
import { 
  ...
  addUsersLocationApi, 
  updateUsersLocationApi, 
  deleteUsersLocationApi 
} from '../../api/general';
    ...
    return locations.map((loc, index) => {
      ...
      return (
        <Marker
          ...
          anchor="bottom"
          onClick={e => { e.originalEvent.stopPropagation(); setSelectedLoc(loc); }}
        >
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      ...
      >
        ...
        <GeolocateControl position="top-right" positionOptions={{ enableHighAccuracy: true }} trackUserLocation={true} showUserLocation={false} />

        {/* סמן של לחיצה על המפה ליצירת נקודה */}
        {pinnedLocation && (
          <Marker latitude={pinnedLocation.latitude} longitude={pinnedLocation.longitude}>
            <div style={{ color: 'red', fontSize: '32px', cursor: 'pointer' }} onClick={() => setShowAddPrompt(true)}>📍</div>
          </Marker>
        )}

        {pinnedLocation && showAddPrompt && (
          <Popup latitude={pinnedLocation.latitude} longitude={pinnedLocation.longitude} anchor="top" onClose={() => setShowAddPrompt(false)} closeOnClick={false}>
            <div style={{ direction: 'rtl', textAlign: 'center', padding: '5px', fontFamily: 'sans-serif' }}>
              ...
              <button
                ...
                style={{ padding: '6px 12px', backgroundColor: '#34a853', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}
              >
                ➕ הוסף נקודה
              </button>
            </div>
        ...
        {/* מיקום המשתמש הנוכחי */}
        {userPosition && (
          <Marker latitude={userPosition.latitude} longitude={userPosition.longitude} anchor="bottom" pitchAlignment="viewport" style={{ zIndex: 10005 }}>
            <div className="user-gps-pulse"><div style={{ fontSize: '34px', cursor: 'pointer' }}>🚶‍♂️</div></div>
          </Marker>
        )}
        ...
        {selectedLoc && (
          <Popup latitude={Number(selectedLoc.latitude)} longitude={Number(selectedLoc.longitude)} anchor="top" onClose={() => setSelectedLoc(null)}>
            <div style={{ direction: 'rtl', textAlign: 'right', fontFamily: 'sans-serif', padding: '5px', width: '210px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', marginTop: '10px', borderTop: '1px solid #eee', paddingTop: '6px', alignItems: 'center' }}>
                <a 
                ...
                <button onClick={() => openEditModal(selectedLoc)} style={{ padding: '2px 6px', backgroundColor: '#fbbc05', color: 'black', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '11px', marginRight: 'auto' }}>✏️ ערוך</button>
                <button onClick={() => onDelete(selectedLoc.id || selectedLoc._id)} style={{ padding: '2px 6px', backgroundColor: '#d93025', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: '11px' }}>🗑️ מחק</button>
              </div>
            </div>
          </Popup>
        )}
      ...
      {/* מודל להוספה / עריכה */}
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999, direction: 'rtl' }}>
          <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '8px', maxWidth: '350px', width: '100%', fontFamily: 'sans-serif', textAlign: 'right', margin: 'auto' }}>
            ...
            <form onSubmit={handleModalSubmit}>
              ...
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 'bold', marginBottom: '4px' }}>תמונה של המקום:</label>
                <input 
                  type="file" 
                  accept="image/*" 
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button type="submit" style={{ flex: 1, padding: '8px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
                  {isEditing ? "עדכן" : "שמור"}
                </button>
                <button type="button" onClick={() => { setShowModal(false); setPinnedLocation(null); setShowAddPrompt(false); }} style={{ padding: '8px', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                  ביטול
                </button>
              </div>
            </form>
